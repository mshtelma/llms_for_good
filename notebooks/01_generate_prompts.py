    ...
    response = deploy_client.predict(
        ...
        inputs={
            "messages": [
                {
                    "role": "system",
                    "content": "You are an AI assistant that specializes in food. "
                ...
                {
                    "role": "user",
                    "content": f"Give me a question related to the following topic: {', '.join(random.sample(topic_list,2))}.",
                },
            ]
        },
    )
...
async def main(url, token, topic, session):
    ...
    body = {"dataframe_records": [{"prompt": prompt(topic)}]}
...
while len(questions) < 10000:
    ...
    results = await run_concurrent_requests(
        PROMPT_LLM_ENDPOINT_URL, PROMPT_LLM_ENDPOINT_TOKEN, topics
    )

    try:
        results = [
            results[i]["predictions"][0]["candidates"][0]["text"]
